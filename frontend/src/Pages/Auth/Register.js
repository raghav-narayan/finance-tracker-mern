...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const nameRegex = /^[a-zA-Z]+ [a-zA-Z]+$/;  // Simple name validation (first and last name)
  
    // Check email format
    if (!emailRegex.test(email)) {
      toast.error("Invalid email format. Please enter a valid email address.", toastOptions);
      return;
    }
    ...
    if (!passwordRegex.test(password)) {
      toast.error(
        "Password must be at least 6 characters long and include a lowercase letter, uppercase letter, number, and special character.",
        toastOptions
      );
      return;
    }
  
    // Check name format (optional, adjust regex for your needs)
    if (!nameRegex.test(name)) {
      toast.error("Please enter your full name (first and last name).", toastOptions);
      return;
    }
    ...
    try {
      ...
      setLoading(false);  // Set loading state after API call (regardless of success/failure)
    }
  };
...
/>
      ...
      <Row>
        ...
        <h1 className="text-center text-white">Your Own Finance Assistant</h1>
        <Col md={{ span: 6, offset: 3 }}>
          ...
          <Form>
            ...
            <div style={{width: "100%", display: "flex" , alignItems:"center", justifyContent:"center", flexDirection: "column"}} className="mt-4">
              ...
              variant="success"
                  type="submit"
                  className=" text-center mt-3 btnStyle success"
